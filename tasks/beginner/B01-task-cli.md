...
- 如果第一天诊断低于 8 分，先完成 [B00 基础起步门](../../foundation/README.md)，不直接阅读整个 B01。
- 一周只做一条可见切片：先实现 `complete`，再由导师确认是否继续 `delete`；不要同时做 Stretch。
- 开始时只打开 `service.py`、对应公共测试和本卡的第一条用例；完成一次小改动后再扩展到存储和 CLI。
- 导师只需提供一个失败测试、一个运行命令和一次中途检查，不需要逐行讲解全部代码。
